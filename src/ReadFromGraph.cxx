...
// Returns false when no loop was detected starting from node n (and m_node_data[n].m_path_condition_per_event is empty).
// Otherwise returns true and m_node_data[n].m_path_condition_per_event
...
// return true and m_node_data[4].m_path_condition_per_event will contain
// the pair {Event(causal_loop, 1), E} (lets abbreviate this to {1,E}).
// Therefore, the call to dfs(6) will return true and cause
// m_node_data[6].m_path_condition_per_event to contain {1,HE},
// the call to dfs(5) will return true and cause
// m_node_data[5].m_path_condition_per_event to contain {1,GHE},
// and the call to dfs(3) will return true and cause
// m_node_data[3].m_path_condition_per_event to contain {1,DE + FGHE}.
// The call to dfs(1) will return true and cause
// m_node_data[1].m_path_condition_per_event to contain {1, BCDE + BCFGHE}
// and m_loop_condition to be updated to BCDE + BCFGHE.
// Finally, the call to dfs(0) will return true and but cause
// m_node_data[0].m_path_condition_per_event to remain empty because
...
// m_node_data[4].m_path_condition_per_event is already contains {1,E}; then
...
// marked as being 'followed' (or rather, the Event(causal_loop, 1) will
...
bool ReadFromGraph::dfs(int current_memory_location)
{
  ...
  // Reset any old values in m_path_condition_per_event (from a previous generation).
  m_node_data[m_current_node].m_path_condition_per_event.reset();
  ...
  for (DirectedSubgraph const* subgraph : m_current_subgraphs)
  {
    for (auto directed_edge = subgraph->edges(m_current_node).begin_outgoing(); directed_edge != subgraph->edges(m_current_node).end_outgoing(); ++directed_edge)
    {
      ...
      else
      {
        ...
        bool have_events = is_cycle(child);
        if (!have_events)
        {
          SequenceNumber current_node{m_current_node};
          m_current_node = child;
          have_events = dfs(current_memory_location);
  ...
  if (have_events)
  {
    ...
    boolean::Expression loop_condition = m_node_data[m_current_node].m_path_condition_per_event.current_loop_condition(m_current_node);
